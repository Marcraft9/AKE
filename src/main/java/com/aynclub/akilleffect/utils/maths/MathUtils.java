package com.aynclub.akilleffect.utils.maths;
...
public class MathUtils {
    ...
    public static Vector getLeftVector(Location loc) {
        float newX = (float) (loc.getX() + Math.cos(Math.toRadians(loc.getYaw() + 0.0f)));
        float newZ = (float) (loc.getZ() + Math.sin(Math.toRadians(loc.getYaw() + 1.8)));
        return new Vector(newX - loc.getX(), 0.0, newZ - loc.getZ());
    }
